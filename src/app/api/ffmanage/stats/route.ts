...
export async function GET(request: Request) {
  ...
  const [
    ...
    db.from("billing_payments").select("amount").eq("payment_status", "success").gte("paid_at", startOfDay),
    db.from("billing_payments").select("amount").eq("payment_status", "success").gte("paid_at", startOfWeek),
    db.from("billing_payments").select("amount").eq("payment_status", "success").gte("paid_at", startOfMonth),
    db.from("billing_payments").select("amount").eq("payment_status", "success").gte("paid_at", startOfYear),
    db.from("support_tickets").select("id", { count: "exact", head: true }).in("status", ["open", "in_progress"]),
    db.from("billing_payments")
  ...
  // Revenue trend (current month, day-by-day)
