...
import { cancelPendingDowngrade } from "@/lib/billing/subscription-service";
...
export async function POST(request: Request) {
  try {
    ...
    if (!sub?.pending_plan_slug) {
      return NextResponse.json({ error: "No pending downgrade to cancel" }, { status: 409 });
    }

    await cancelPendingDowngrade(admin, {
      workspaceId,
      userId: user.id,
    ...
    console.error("[billing/cancel-downgrade]", err);
