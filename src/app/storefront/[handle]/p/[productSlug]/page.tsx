...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = product.description?.trim().slice(0, 160) || `Buy ${product.name} from ${resolution.store.storeName} on FundiFlow.`;
  const images = product.images?.filter((image) => isAllowedImageUrl(image.url)).map((image) => ({ url: image.url, alt: image.altText ?? product.name }));
  return {
...
export default async function StorefrontProductPage({ params }: Props) {
  ...
  const images = product.images?.filter((image) => isAllowedImageUrl(image.url)) ?? [];
